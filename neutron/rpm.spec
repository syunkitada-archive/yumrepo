...
opt/%{name}/bin/pip install -r base/requirements.txt
...
mv %{buildroot}/etc/%{name}/%{name}/plugins %{buildroot}/etc/%{name}/
mv %{buildroot}/etc/%{name}/%{name}/rootwrap.d %{buildroot}/etc/%{name}/
rm -rf %{buildroot}/etc/%{name}/%{name}
rm -rf %{buildroot}/etc/%{name}/oslo-config-generator
...
%dir %attr(0755, root, root) /etc/%{name}/plugins
%dir %attr(0755, root, root) /etc/%{name}/rootwrap.d
%config(noreplace) %attr(-, root, root) /etc/%{name}/*
%config(noreplace) %attr(-, root, root) /etc/%{name}/plugins/*
